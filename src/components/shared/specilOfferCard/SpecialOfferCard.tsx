  ...
  return (
    <>
      <TouchableWithoutFeedback key={index} onPress={() => setSelected(index)}>
        ...
        >
          <FeatherIcon color={isActive ? '#F82E08' : '#363636'} name={isActive ? 'check-circle' : 'circle'} size={24} />
            ...
            <View>
              ...
              {timeRemaining > 0 ? (
                <Text style={[styles.radioText, { fontSize: 14, color: COLORS.primary, marginLeft: -5 }]}>
                  <Image
                    source={{
                      uri: 'https://threedio-prod-var-cdn.icons8.com/yn/preview_sets/previews/x1kHx-Bof0L_2XOh.webp',
                    }}
                    style={{ height: 18, width: 20 }}
                  />{' '}
                  {formatTime(timeRemaining)} minutes left
                </Text>
              ) : (
